export class Validator {
  static #errorHandler = (msg, status) => {
    const error = new Error(msg)
    error.status = status
    return error
  }

  static validateJug = (req, res, next) => {
  ...
  static checkFeasibility = (req, res, next) => {
    const { x_capacity, y_capacity, z_amount_wanted } = req.body
  ...
  static #gcd (a, b) { // This function implements Euclid's algorithm to find the GCD
    while (b !== 0) {
      const temp = b
      b = a % b
      a = temp
